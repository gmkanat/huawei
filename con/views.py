...
from courier.models import OTP
...
class ConViewSet(viewsets.mixins.CreateModelMixin,
    ...
    def send_otp(self, request):
        ...
        order = validate_order(order_id, 'in_progress', courier_id)
    ...
    def verify_otp(self, request):
        ...
        order_id = request.query_params.get('order_id')
        order = validate_order(order_id, 'in_progress', request.user.id)
        if not otp_code:
            return Response(
        ...
        response = send_sms(phone, f'Your order is on the way, {order.courier_id.name} {order.courier_id.phone}')
        if response.status_code != 200:
            return Response(
                {'error': 'SMS not sent'},
                status=status.HTTP_400_BAD_REQUEST
